...
def test_current_requirement_receipts_can_advance_without_collapsing_parent_acceptance():
    manifest = load("governance/canonical_scope_manifest.json")
    retained = set(manifest["retained_requirement_ids"])
    ...
    assert required_current <= set(current)
    assert set(current) <= retained
    ...
    assert current["REQ-SCOPE-079"]["claims"]["REACHABLE"]["state"] == "PROVEN"
    ...
    assert current["REQ-SCOPE-090"]["claims"]["REACHABLE"]["state"] == "PROVEN"
