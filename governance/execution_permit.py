...
def _b64decode(value: str) -> bytes:
    ...
    try:
        ...
        raise ExecutionPermitError(
            "malformed execution permit encoding"
        ) from exc
...
class SQLitePermitLedger:
    ...
    def __init__(self, path: str | Path):
        ...
        if not self.path.is_absolute():
            raise ExecutionPermitError(
                "permit ledger path must be absolute"
            )
        ...
        *,
        ...
        try:
            with sqlite3.connect(
                ...
                conn.execute(
                    "INSERT INTO consumed_execution_permits"
                    "(permit_id,consumed_at,envelope_fingerprint,"
                    "action_intent_fingerprint) VALUES(?,?,?,?)",
                    (
                        permit_id,
                        consumed_at,
...
class ExecutionPermitAuthority:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(secret, bytes) or len(secret) < 32:
            raise ExecutionPermitError(
                "execution permit secret must be at least 32 bytes"
            )
    ...
    def _decode(self, token: str):
        ...
        try:
            ...
            raise ExecutionPermitError(
                "execution permit payload is invalid"
            ) from exc
        if payload.get("version") != self.VERSION:
            raise ExecutionPermitError(
                "unsupported execution permit version"
            )
        return payload
    ...
    def _validate_action_binding(
        ...
        envelope_class = str(
            envelope["authority"]["action_class"]
        ).strip().upper()
        if envelope_class != action.action_class:
            raise ExecutionPermitError(
                "action class mismatch "
                f"envelope={envelope_class} "
                f"action={action.action_class}"
            )

    @staticmethod
    def _validate_required_human_approval(
        envelope,
        action: ActionIntent,
        ...
        if approval is None:
            raise ExecutionPermitError(
                "human-required action lacks exact approval binding"
            )
        validation = AuthorityService.validate(action, approval)
        if validation.status != "VALID":
            raise ExecutionPermitError(
                "human approval is stale for the current action intent"
            )
        try:
            require_trusted_approval(
                snapshot,
            ...
            raise ExecutionPermitError(str(exc)) from exc
    ...
    def issue(
        ...
        *,
        ...
        if action.action_class == "READ_ONLY":
            raise ExecutionPermitError(
                "read-only actions do not require stateful execution permits"
            )
        self._validate_required_human_approval(
            envelope,
            action,
            approval,
            snapshot,
        ...
        payload = {
            ...
            "authority_source_ref": str(
                envelope["authority"]["source_ref"]
            ).strip(),
        }
        payload_bytes = _canonical(payload)
        token = (
            f"{_b64encode(payload_bytes)}."
            f"{_b64encode(self._sign(payload_bytes))}"
        )
        return IssuedExecutionPermit(
            token=token,
            **{
                key: payload[key]
                for key in (
        ...
        *,
        ...
        for field, current in expected.items():
            if payload.get(field) != current:
                raise ExecutionPermitError(
                    f"execution permit no longer matches {field}"
                )

        permit_id = str(payload.get("permit_id", "")).strip()
        if not permit_id:
            raise ExecutionPermitError(
                "execution permit has no permit_id"
            )
