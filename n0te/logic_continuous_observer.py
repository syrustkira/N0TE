...
class LogicContinuousObserverError(RuntimeError):
    """Continuous Logic observation could not preserve trustworthy session state."""


def _finite_positive(value: object, field: str, *, maximum: float) -> float:
    if isinstance(value, bool):
        raise LogicContinuousObserverError(f"{field} must be numeric")
    try:
        number = float(value)
    except (TypeError, ValueError) as exc:
        raise LogicContinuousObserverError(f"{field} must be numeric") from exc
    if not math.isfinite(number) or number <= 0 or number > maximum:
        raise LogicContinuousObserverError(
...
def _text(value: object, field: str) -> str:
    ...
    if not text:
        raise LogicContinuousObserverError(f"{field} must not be empty")
    return text


def _observation_fingerprint(snapshot: LogicTimingSnapshot) -> tuple[object, ...]:
    return (
        ...
        snapshot.is_playing,
        snapshot.is_cycling,
        snapshot.meter_numerator,
        snapshot.meter_denominator,
    )


def _reference_fingerprint(snapshot: LogicTimingSnapshot) -> tuple[object, ...]:
...
class LogicContinuousObservationCycle:
    sequence: int
    ...
    def __post_init__(self) -> None:
        if self.sequence < 1:
            raise LogicContinuousObserverError("sequence must be positive")
        if not isinstance(self.snapshot, LogicTimingSnapshot):
            raise TypeError("snapshot must be LogicTimingSnapshot")
...
class LogicContinuousObserver:
    """Keep N0TE aware of Logic timing without turning transport churn into search."""
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(client, LogicMidiMonitorClient):
            raise TypeError("client must be LogicMidiMonitorClient")
        ...
        for name, values in (
            ...
            if any(not value for value in values):
                raise LogicContinuousObserverError(f"{name} contains an empty value")
        ...
        self._sequence = 0
        self._latest_snapshot: LogicTimingSnapshot | None = None
    ...
    @property
    def latest_snapshot(self) -> LogicTimingSnapshot | None:
    ...
    async def poll_once(
        ...
        *,
        ...
        return LogicContinuousObservationCycle(
    ...
    ) -> AsyncIterator[LogicContinuousObservationCycle]:
