...
class FLStudioSessionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not data.is_absolute() or not state.is_absolute() or not snapshot.is_absolute():
            raise FLStudioSessionLauncherError("session and snapshot paths must be absolute")
        if not str(self.profile_id).strip():
            raise FLStudioSessionLauncherError("profile_id must not be empty")
        if not str(self.provider_id).strip():
            raise FLStudioSessionLauncherError("provider_id must not be empty")
...
def resolve_snapshot_path(
    *,
    ...
    raw = explicit_snapshot if explicit_snapshot is not None else environment.get(SNAPSHOT_PATH_ENV)
    if raw is not None and str(raw).strip():
        path = Path(str(raw)).expanduser()
        if not path.is_absolute():
            raise FLStudioSessionLauncherError("FL Studio snapshot path must be absolute")
        return path
    ...
    return user_data / "Settings" / "Hardware" / SCRIPT_DIR_NAME / SNAPSHOT_FILE_NAME
...
def run_session(
    ...
    *,
    ...
    try:
        ...
        if song is None:
            raise FLStudioSessionLauncherError("selected profile has no active Song")
        ...
        output(f"N0TE FL Studio session • {song.title} • coordinator {status.lower()}")
        if not config.once:
            output(
                "N0TE FL bridge • reconnect is automatic while the FL snapshot is unavailable"
            )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m n0te.fl_studio_session_launcher")
